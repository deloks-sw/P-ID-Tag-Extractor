...
const DeleteRelationshipButton = React.memo(({ onClick }: { onClick: () => void }) => (
  <button
    ...
    className="ml-2 p-0.5 rounded-full text-gray-500 hover:bg-red-500/20 hover:text-red-400 transition-colors"
    title="관계 삭제"
  >
...
const DeleteTagButton = React.memo(({ onClick }: { onClick: () => void }) => (
  <button
    ...
    className="p-1 rounded-full text-gray-500 hover:bg-red-500/20 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
    title="태그 삭제"
  >
...
const EditButton = React.memo(({ onClick, title = "편집" }: { onClick: () => void; title?: string }) => (
  <button
    onClick={(e) => {
      e.stopPropagation();
      onClick();
    }}
    className="p-1 rounded-full text-gray-500 hover:bg-sky-500/20 hover:text-sky-400 transition-colors opacity-0 group-hover:opacity-100"
    title={title}
  >
...
const SaveButton = React.memo(({ onClick, title = "저장" }: { onClick: () => void; title?: string }) => (
  <button
    onClick={(e) => {
      e.stopPropagation();
      onClick();
    }}
    className="p-1 rounded-full text-green-600 hover:text-green-700 transition-colors"
    title={title}
  >
    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
...
const CancelButton = React.memo(({ onClick, title = "취소" }: { onClick: () => void; title?: string }) => (
  <button
    onClick={(e) => {
      e.stopPropagation();
      onClick();
    }}
    className="p-1 rounded-full text-gray-600 hover:text-gray-700 transition-colors"
    title={title}
  >
    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
      <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
  ...
  return (
    <div className="group flex items-center justify-between" onClick={(e) => {if(isEditing) e.stopPropagation()}}>
      <div className="flex items-center space-x-1.5 flex-grow min-w-0" title={item.text}>
        <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-gray-500 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clipRule="evenodd" /></svg>
        {isEditing ? (
          <input
            ref={inputRef}
            type="text"
            ...
            className="font-mono text-sm text-gray-900 bg-gray-100 border border-sky-500 rounded px-1 w-full"
          />
        ) : (
          <span className="text-gray-700 font-mono truncate max-w-[180px]">{item.text}</span>
        )}
      </div>
...
const TagListItem: React.FC<TagListItemProps> = React.memo(({ tag, isSelected, onItemClick, onGoToTag, relationships, allTags, allRawTextItems, descriptions, loops, onToggleReviewStatus, onDeleteRelationship, onDeleteTag, onUpdateTagText, onDeleteItem, onUpdateItemText, onUpdateLoop, showDetails }) => {
  const [isEditing, setIsEditing] = useState(false);
  ...
  const renderRelationship = (id, text) => (
    <span
      ...
      className="text-sky-600 hover:text-sky-700 hover:underline cursor-pointer font-mono"
    >
      {text}
    </span>
  );
  ...
  return (
    <li
      ...
      className={`group p-2 rounded-md cursor-pointer transition-colors ${isSelected ? 'bg-red-500/20 ring-1 ring-red-500' : 'hover:bg-gray-100'}`}
    >
      <div className="flex justify-between items-start">
          ...
          {isEditing ? (
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center space-x-2 flex-grow min-w-0">
                ...
                />
                <span 
                  className={`inline-flex items-center justify-center w-5 h-5 rounded text-xs font-bold text-gray-900 ${colors.bg} ${colors.border} border flex-shrink-0`}
                  title={tag.category}
                >
                ...
                {tagLoops.map(loop => {
                  ...
                  return (
                    <div key={loop.id} className="mb-1">
                      <div 
                        className="flex items-center justify-between cursor-pointer hover:bg-gray-100 rounded px-1 py-0.5 transition-colors"
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      {isExpanded && (
                        <div className="mt-1 ml-2 p-2 border border-gray-300 rounded-md bg-gray-50">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {loopTags.map(loopTag => (
                              <div
                                key={loopTag.id}
                                className={`inline-flex items-center bg-gray-200 text-gray-700 rounded text-xs font-mono overflow-hidden ${loopTag.id === tag.id ? 'ring-1 ring-blue-400' : ''}`}
                              >
                                <span
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  title={`태그로 이동: ${loopTag.text}`}
                                >
                                  {loopTag.text} <span className="text-gray-600">P.{loopTag.page}</span>
                                </span>
                                {loopTag.id !== tag.id && (
                                  <button
                                    onClick={(e) => {
        ...
        <div className="flex items-center space-x-1 flex-shrink-0">
          <span className="text-xs text-gray-600">페이지 {tag.page}</span>
          <EditTagButton onClick={() => setIsEditing(true)} />
      ...
      {showDetails && hasRelationships && (
        <div className="mt-2 pt-2 border-t border-gray-200/50 space-y-1 text-xs text-gray-700">
          {/* Outgoing Connections */}
          {outgoingConnections.map(rel => {
            const otherTag = tagMap.get(rel.to);
            return otherTag ? <div key={rel.id} className="flex items-center justify-between"><div className="flex items-center space-x-1.5"><span className="text-gray-700 text-xs">대상</span>{renderRelationship(otherTag.id, otherTag.text)}</div><DeleteRelationshipButton onClick={() => onDeleteRelationship(rel.id)} /></div> : null;
          })}
          {/* Incoming Connections */}
          {incomingConnections.map(rel => {
            const otherTag = tagMap.get(rel.from);
            return otherTag ? <div key={rel.id} className="flex items-center justify-between"><div className="flex items-center space-x-1.5"><span className="text-gray-700 text-xs">출처</span>{renderRelationship(otherTag.id, otherTag.text)}</div><DeleteRelationshipButton onClick={() => onDeleteRelationship(rel.id)} /></div> : null;
          })}
          ...
          {installedInstruments.length > 0 && (
            <div>
              <span className="text-gray-600 font-semibold">설치된 계기:</span>
          ...
          {annotationRelationships.length > 0 && (
            <div>
              <span className="text-gray-600 font-semibold">관련 텍스트:</span>
          ...
          {noteRelationships.length > 0 && (
            <div>
              <span className="text-gray-600 font-semibold">노트:</span>
              <div className="pl-3 space-y-1 mt-1">
                {noteRelationships.map(rel => {
                  ...
                  return (
                    <div key={rel.id} className="border border-gray-300 rounded-md bg-gray-50">
                      {/* Note tag header */}
                      ...
                      {isExpanded && noteDescriptions.length > 0 && (
                        <div className="px-3 pb-2 border-t border-gray-300">
                          <div className="space-y-1 mt-2">
                            {noteDescriptions.map(description => {
                              ...
                              return (
                                <div key={description.id} className="border border-gray-300 rounded-md bg-gray-50">
                                  {/* Description header */}
                                  <div className="flex items-center justify-between p-2">
                                    <button
                                      onClick={(e) => {
                                      ...
                                      className="flex items-center space-x-1.5 text-purple-600 hover:text-purple-700 text-xs cursor-pointer"
                                    >
                                      <span>{isDescExpanded ? '📖' : '📄'}</span>
                                      <span>{description.metadata.type} {description.metadata.number}</span>
                                      <span className="text-gray-600">{isDescExpanded ? '▼' : '▶'}</span>
                                    </button>
                                    {descRel && (
                                  ...
                                  {isDescExpanded && (
                                    <div className="px-3 pb-3 border-t border-gray-300">
                                      ...
                                      <div className="text-xs text-gray-600 mt-1">
                                        페이지 {description.page} • {description.metadata.scope}
                                      </div>
                                    </div>
                                  )}
                                </div>
          ...
          {notedByRelationships.length > 0 && (
            <div>
              <span className="text-gray-600 font-semibold">노트 대상:</span>
              <div className="pl-3 space-y-0.5 mt-1">
                    ...
                    <div key={rel.id} className="border border-gray-300 rounded-md bg-gray-50">
                      ...
                      <div className="flex items-center justify-between p-2">
                        <button
                          ...
                          className="flex items-center space-x-1.5 text-purple-600 hover:text-purple-700 text-xs cursor-pointer bg-transparent border-none"
                        >
          ...
          {describedByRelationships.length > 0 && (
            <div>
              <span className="text-gray-600 font-semibold">설명 출처:</span>
              <div className="pl-3 space-y-0.5 mt-1">
...
export const SidePanel = ({
  // Data props
  tags, setTags, rawTextItems, descriptions,
  loops, setLoops, relationships, setRelationships,
  detectedLines = [],
  appSettings = { instrumentMappings: {} } as any, // Add app settings for instrument mappings
  // View props
  ...
  onDeleteRawTextItems, onUpdateRawTextItemText,
  onAutoLinkDescriptions, onAutoLinkNotesAndHolds,
  onAutoGenerateLoops, onManualCreateLoop, onDeleteLoops, onUpdateLoop, showConfirmation,
  // Ping props
  onPingTag, onPingDescription, onPingRelationship,
  // Visibility props
  ...
  const [tempDescriptionMetadata, setTempDescriptionMetadata] = useState<any>({ type: 'Note', scope: 'Installation', number: 1 });
  ...
  const virtualizedTags = useMemo(() => {
    ...
    if (selectedTagIds.length === 1 && !isProgrammaticScrollRef.current) {
      ...
      if (selectedIndex !== -1) {
        ...
        if (isOutsideRange) {
          // Delay update slightly to avoid conflicts with scrolling
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    if (tagSelectionSource === 'pdf' && selectedTagIds.length === 1 && activeTab === 'tags' && listRef.current) {
      ...
      // Set flag to prevent virtualization updates during scroll
      ...
      setTimeout(() => {
        ...
        if (element) {
          // Use instant scroll to avoid conflicts
  ...
  const handleExport = useCallback(() => {
    // Pass true to enable note descriptions from right-side text
    // Pass instrument mappings and loop rules from app settings
    const instrumentMappings = appSettings.instrumentMappings || {};
    const loopRules = appSettings.loopRules || {};
    exportToExcel(tags, relationships, rawTextItems, descriptions, [], loops, [], detectedLines, true, instrumentMappings, loopRules);
  }, [tags, relationships, rawTextItems, descriptions, loops, detectedLines, appSettings]);
  ...
  const handleScroll = useCallback((e) => {
    // Skip virtualization updates during bulk operations or programmatic scrolls
  ...
  const RelationshipViewer = useCallback(({ relationships: inputRelationships }) => {
    ...
    }), [tags, descriptions, rawTextItems]);
    ...
    const handleRelationshipClick = useCallback((relationship) => {
      ...
      if (fromEntity && toEntity) {
        ...
        const targetPage = (fromEntity as any)?.page || (toEntity as any)?.page;
    ...
    }, [getEntity, currentPage, setCurrentPage, entityMaps, onPingTag, onPingDescription, onPingRelationship]);

    return (
        <div className="flex-grow flex flex-col overflow-hidden">
            <div className="p-2 border-b border-gray-200 flex-shrink-0 space-y-2">
                <input
                    type="text"
                    placeholder="관계 검색..."
                    value={relSearchQuery}
                    onChange={(e) => setRelSearchQuery(e.target.value)}
                    className="w-full bg-white border border-gray-300 rounded-md px-2 py-1.5 text-sm text-gray-900 focus:ring-sky-500 focus:border-sky-500"
                />
            ...
            <ul className="flex-grow overflow-y-auto p-2 space-y-2">
                {searchFilteredRelationships.length === 0 && (
                    <li className="text-center text-gray-600 py-4">관계를 찾을 수 없습니다.</li>
                )}
                {searchFilteredRelationships.map(rel => {
                    ...
                    const renderEntity = (entity) => {
                      ...
                      return (
                        <span
                          onClick={handleClick}
                          className="font-mono text-sky-600 hover:underline cursor-pointer"
                        >
                          {text}
                        </span>
                      );
                    ...
                    return (
                        <li 
                          key={rel.id} 
                          className="p-2 bg-gray-100 rounded-md text-sm text-gray-700 flex items-center justify-between hover:bg-gray-200 cursor-pointer transition-colors"
                          onClick={() => handleRelationshipClick(rel)}
                          title={`PDF에서 강조 표시: ${title}`}
                        >
                            <div className="flex items-center space-x-2 flex-grow">
                                <span className="text-xs text-gray-500 font-medium">{rel.type}</span>
                                <span className="text-gray-700 text-xs">출처</span>
                                {renderEntity(fromEntity)}
                                <span className="text-gray-700 text-xs">대상</span>
  ...
  }, [tags, descriptions, rawTextItems, currentPage, setCurrentPage, onPingTag, onPingDescription, onPingRelationship, goToTag, handleDeleteRelationship]);

  const filterCategories = ['All', Category.Line, Category.Instrument, Category.NotesAndHolds, Category.DrawingNumber];
  ...
  return (
    <aside 
      className="h-full bg-white border-r border-gray-200 flex flex-col flex-shrink-0 relative"
      style={{ width: `${sidebarWidth}px` }}
    >
      
      <div className="border-b border-gray-200 flex text-xs">
        <button onClick={() => setActiveTab('tags')} className={`flex-1 py-2 px-1 font-semibold ${activeTab === 'tags' ? 'bg-gray-100 text-sky-600' : 'text-gray-700'}`}>태그 ({totalTagCount})</button>
        <button onClick={() => setActiveTab('descriptions')} className={`flex-1 py-2 px-1 font-semibold ${activeTab === 'descriptions' ? 'bg-gray-100 text-sky-600' : 'text-gray-700'}`}>노트 ({filteredDescriptions.length})</button>
        <button onClick={() => setActiveTab('loops')} className={`flex-1 py-2 px-1 font-semibold ${activeTab === 'loops' ? 'bg-gray-100 text-sky-600' : 'text-gray-700'}`}>루프 ({filteredLoops.length})</button>
        <button onClick={() => setActiveTab('relationships')} className={`flex-1 py-2 px-1 font-semibold ${activeTab === 'relationships' ? 'bg-gray-100 text-sky-600' : 'text-gray-700'}`}>관계 ({filteredRelationships.length})</button>
      </div>
      ...
      {activeTab === 'tags' && (
        <div className="flex-grow flex flex-col overflow-hidden">
            <div className="p-3 space-y-2 border-b border-gray-200 flex-shrink-0">
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-white border border-gray-300 rounded-md px-2 py-1.5 pr-8 text-sm text-gray-900 focus:ring-sky-500 focus:border-sky-500"
                  />
                  {searchQuery && (
                    <button
                ...
                <div className="border-b border-gray-200 flex text-xs">
                  {filterCategories.map((cat, index) => {
                    ...
                    const shortName = cat === 'Equipment' ? '장비' 
                      : cat === 'Instrument' ? '계기'
                      : cat === 'DrawingNumber' ? '도면번호'
                      : cat === 'NotesAndHolds' ? '노트'
                      : cat === 'SpecialItem' ? '특수'
                      : cat === 'All' ? '전체' : cat;
                    ...
                    return (
                      <button 
                        ...
                        className={`flex-1 py-1.5 px-1 font-semibold text-center border-r border-gray-300 last:border-r-0 ${
                          isActive ? 'bg-gray-100 text-sky-600' : 'text-gray-700'
                        }`} 
                        disabled={count === 0 && cat !== 'All'}
                      >
                        <div className="flex flex-col items-center">
                          <span className="text-xs leading-tight text-gray-700">{shortName}</span>
                          <span className="text-xs text-gray-600 leading-tight">({count})</span>
                        </div>
                      </button>
                    );
                  })}
            ...
            {selectedTagIds.length > 1 && (
              <div className="p-2 flex-shrink-0">
                  ...
                  >
                    ...
                    <span>선택한 항목 삭제 ({selectedTagIds.length})</span>
                  </button>
              </div>
            )}
            ...
            <ul 
              ref={listRef} 
              className="flex-grow overflow-y-auto p-2 divide-y divide-gray-200"
          ...
          <div ref={descriptionListRef} className="flex-grow overflow-y-auto p-3 space-y-2">
            {filteredDescriptions.length === 0 ? (
              <div className="text-center text-gray-500 mt-8">
                ...
                <div className="text-sm text-gray-600">{showCurrentPageOnly && descriptions.length > 0 ? `${currentPage} 페이지에 설명이 없습니다` : '아직 설명이 없습니다'}</div>
              ...
              filteredDescriptions.map((description) => {
                ...
                <div 
                  ...
                  className={`group border rounded-lg p-3 hover:bg-gray-100 transition-colors ${
                    isSelected ? 'bg-purple-100 border-purple-400' : 'bg-gray-50 border-gray-300'
                  }`}
                >
                      ...
                      {relatedNoteTags.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {relatedNoteTags.map(noteTag => (
                            ...
                            >
                              <span>📝</span>
                              <span className="font-mono text-gray-700">{noteTag.text}</span>
                            </span>
                          ))}
                        </div>
                      )}
                  ...
                  {isEditing ? (
                    ...
                    <div className="space-y-2">
                      <textarea
                        ...
                        className="w-full bg-white border border-gray-300 rounded px-2 py-1 text-sm text-gray-900 resize-none"
                        rows={3}
        ...
        <div className="flex-grow flex flex-col overflow-hidden">
          <div className="p-3 space-y-2 border-b border-gray-200">
            <input
              ...
              className="w-full bg-slate-900 border border-gray-300 rounded-md px-2 py-1.5 text-sm focus:ring-sky-500 focus:border-sky-500"
            />
            <div className="text-xs text-gray-600">
          ...
          <div className="flex-grow overflow-y-auto p-3 space-y-2">
            {filteredLoops.length === 0 ? (
              <div className="text-center text-gray-500 mt-8">
                <div className="text-lg mb-2">🔗</div>
                <div className="text-sm">아직 생성된 루프가 없습니다</div>
              ...
              filteredLoops
                ...
                .map(loop => {
                  ...
                  return (
                    <div 
                      key={loop.id} 
                      className="group border border-gray-300 rounded-lg p-3 bg-slate-700/30 cursor-pointer hover:bg-slate-600/30 transition-colors"
                    ...
                    >
                          ...
                          {editingLoopId === loop.id ? (
                            <div className="flex items-center space-x-2 flex-grow">
                              <input
                                ...
                                className="bg-slate-800 border border-gray-300 rounded px-2 py-1 text-sky-300 font-mono font-semibold text-sm flex-grow focus:ring-sky-500 focus:border-sky-500"
                                autoFocus
                                onClick={(e) => e.stopPropagation()}
                              />
                            ...
                            <>
                              <span className="text-sky-600 font-mono font-semibold text-sm">{loop.name || loop.id}</span>
                              <span className="text-xs text-gray-600">
                                ({loopTags.length} tags)
                                {loopTags.length > 0 && ` P. ${loopTags.map(t => t.page).filter((v, i, a) => a.indexOf(v) === i).sort((a, b) => a - b).join(', ')}`}
                              </span>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {loopTags.map(tag => (
                          <div
                            key={tag.id}
                            className="inline-flex items-center bg-gray-200 text-gray-700 rounded text-xs font-mono overflow-hidden"
                          >
                            <span
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            >
                              {tag.text} <span className="text-gray-600">P.{tag.page}</span>
                            </span>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
      ...
      >
        <div className="absolute top-1/2 right-0 transform -translate-y-1/2 translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
          <div className="flex flex-col items-center justify-center w-3 h-8 bg-slate-700 border border-gray-300 rounded-sm">
